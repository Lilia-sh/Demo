...
public class LibraryAPI {
    public static Author[] getAuthorsData() {
        ...
        for (int i = 0; i < authors.length; i++) {
            Author author = new Author(LibraryDataRepo.names[(int) (Math.random() * 7)], LibraryDataRepo.surnames[(int) (Math.random() * 7)], (int) (Math.random() * 100));
            authors[i] = author;
        }
        return authors;
    }
    ...
    public static Book[] getBookData(Author authors[]) {
        ...
        for (int i = 0; i < books.length; i++) {
            Book book = new Book(LibraryDataRepo.titles[(int) (Math.random() * 5)],
                    authors[(int) (Math.random() * authors.length)],
                    Book.createRandomDate(1900, 2000),
                    LibraryDataRepo.categories[(int) (Math.random() * 5)],
